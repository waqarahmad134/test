...
<div class="card h-100 p-0 radius-12">
    <div class="card-header border-bottom bg-base py-16 px-24 d-flex align-items-center flex-wrap gap-3 justify-content-between">
        <div class="d-flex align-items-center flex-wrap gap-3">
            <form action="{{ route('tehsils.index') }}" method="GET" class="d-flex gap-2">
        ...
        <a href="{{ route('tehsils.create') }}" class="btn btn-primary text-sm btn-sm px-12 py-12 radius-8 d-flex align-items-center gap-2">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            @if($tehsil->status == 'active')
                            <form action="{{ route('tehsils.toggleStatus', $tehsil->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('POST')
                                <button type="submit">
                        ...
                        <td class="text-center">
                            <div class="d-flex align-items-center gap-10 justify-content-center">
                                <a href="{{ route('tehsils.edit', $tehsil->id) }}" class="bg-success-focus text-success-600 bg-hover-success-200 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle">
                                    <iconify-icon icon="lucide:edit" class="menu-icon"></iconify-icon>
                                </a>
                                <form action="{{ route('tehsils.destroy', $tehsil->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mt-24 w-100 ">
            <nav aria-label="Page navigation example w-100" style="width: 100%;">
                <ul class="pagination justify-content-center mb-0">
                    {{ $tehsils->links('pagination::bootstrap-5') }}
                </ul>
            </nav>
        </div>
