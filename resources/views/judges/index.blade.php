...
<div class="section__content section__content--p30">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <h5 class="title-5 mb-4">Courts Management</h5>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="card-body p-24">
            ...
            <table class="table bordered-table sm-table mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $judge->name }}</td>
                        <td>
                            <button class="btn btn-primary btn-sm editProduct" data-id="{{ $judge->id }}">Edit</button>
                            <button class="btn btn-danger btn-sm deleteProduct" data-id="{{ $judge->id }}">Delete</button>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="3" class="text-center">No courts found</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mt-24 w-100">
            <p class="text-sm text-secondary-light mb-0">
                Showing {{ $judges->count() }} of {{ $judges->count() }} entries
            </p>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $('#createNewProduct').click(function () {
            ...
            $('#myModalLabel').html("Create New Court");
        ...
        $(document).on('click', '.editProduct', function () {
            ...
            $.get("{{ route('judges.index') }}" + '/' + product_id + '/edit', function (data) {
                $('#myModalLabel').html("Edit Court");
        ...
        $(document).on('click', '.deleteProduct', function () {
            ...
            $.ajax({
                type: "DELETE",
                url: "{{ route('judges.store') }}" + '/' + product_id,
                success: function (data) {
                    // Reload page to show updated data
                    location.reload();
                },
