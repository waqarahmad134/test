    ...
    <div class="card-body p-24">
        <div class="table-responsive scroll-sm">
            <table class="table bordered-table sm-table mb-0">
                ...
                <tbody>
                    @forelse($pss as $index => $ps)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $ps->name }}</td>
                        <td>
                            <button class="btn btn-primary btn-sm editProduct" data-id="{{ $ps->id }}">Edit</button>
                            <button class="btn btn-danger btn-sm deleteProduct" data-id="{{ $ps->id }}">Delete</button>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="3" class="text-center">No police stations found</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mt-24 w-100">
            <p class="text-sm text-secondary-light mb-0">
                Showing {{ $pss->count() }} of {{ $pss->count() }} entries
            </p>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myModalLabel">Case Details</h5>
            ...
            <div class="modal-body">
                <form id="productForm" name="productForm" class="needs-validation" novalidate>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" placeholder="Enter Station Name" class="form-control" required>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#createNewProduct').click(function () {
            ...
            $('#myModalLabel').html("Create New Police Station");
        ...
        $(document).on('click', '.editProduct', function () {
            ...
            $.get("{{ route('ps.index') }}" + '/' + product_id + '/edit', function (data) {
                $('#myModalLabel').html("Edit Police Station");
